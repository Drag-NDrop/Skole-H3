...
namespace BaggageSortering.Classes
{
    internal class ConsumerWithPulse
    {
        ...
        {
            while (/*MainWindow.ShouldStop !=*/ true)
            {
                Baggage unit = _buffer.Remove();
                UnitsConsumed++;
                Console.WriteLine($"Consumer_{unit.Destination} : Picked up and consumed baggage for {unit.Destination}...");
            }
