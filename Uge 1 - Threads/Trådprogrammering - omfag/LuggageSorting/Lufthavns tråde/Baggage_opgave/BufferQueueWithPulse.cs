...
namespace BaggageSortering.Classes
{
    ...
    /// </summary>
    internal abstract class BufferQueueWithPulse : INotifyPropertyChanged
    {
        ...
        private Queue<Baggage> _bufferQueue = new Queue<Baggage>();
        private bool _verbose;
        private int _id;
        private string _type;
        private string _destination;
        private string _bufferUnits = "0";
        ...
        public string BufferUnits
        {
            ...
            set
            {
                _bufferUnits = value;
                OnPropertyChanged();
                Debug.WriteLine($"[Buffer]{Type} #{Id} - {_bufferUnits}");
            }
        }
        ...
        public BufferQueueWithPulse(int id, string destination, bool verbose, string type, int maxCapacity)
        {
            Id = id;
        ...
        public void Add(Baggage unit)
        {
            try
            {
                lock (_lock)// Waits until it can enter the lock. Thread is now locked.
                {
                    ...
                    {
                        Console.WriteLine($"[Buffer]{Type} #{Id} - Destination: {_destination} : Awaiting {_type} consumption.");
                    ...
                    if (_verbose)
                    {
                        Console.WriteLine($"[Buffer]{Type} #{Id} - Destination: {_destination} : had item #{unit.Id} added..");
                    }
                    BufferUnits = _bufferQueue.Count.ToString();
        ...
        public Baggage Remove()
        {
            ...
            try
            {
                ...
                { // Thread is now locked.
                    if (_bufferQueue.Count > 0)
                    {
                        ...
                        if (_verbose)
                        {
                            Console.WriteLine($"[Buffer]{Type} #{Id} Destination: {unit.Destination}, had item #{unit.Id} flown away..");
                        }
                    }
                    else
                    {
                        Console.WriteLine($"[Buffer]{Type} #{Id} - Destination: {_destination} : Could not remove unit. It's empty");
                        while (_bufferQueue.Count <= 0)
                        { // Halt the flow if the buffer is empty
                            Console.WriteLine($"[Buffer]{Type} #{Id} - Destination: {_destination} : Is empty. Awaiting more luggage..");
                            Monitor.Wait(_lock);
                        }
            ...
            catch (Exception e)
            {
                ...
                Console.WriteLine($"Warning: [Buffer]{Type} #{Id} - Destination: {_destination} : encountered an error, and returned a null-value unit.");
                return null;
            }
